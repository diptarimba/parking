...
@section('page-content')
    <x-card.layout mainTitle="User" subTitle="Update User Location">
        <x-slot name="header">
            <span class="h4 text-bold">Update User Location [ {{ $user->name }} ] </span>
        </x-slot>
        <x-slot name="body">
            <form id="form" action="{{ route('user.location.update', $user->id) }}" class="theme-form mega-form"
                method="post" enctype="multipart/form-data">
                @csrf
                <x-forms.put-method />
                <div class="mb-2">
                    <label class="form-label" for="datalistOptions">Lokasi</label>
                    <select id="datalistOptions" class="form-select chosen-select" multiple name="location[]"
